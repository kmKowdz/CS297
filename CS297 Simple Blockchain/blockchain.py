...
def new_transaction():
    ...
    if len(blockchain.current_transactions)<AverageTransactionsPerBlock:
        ...
        response = {'message': f'Block is full. Transactions will be not added to Block {index}. Your new transactions will be added to Block{index+1} after mining.'}
        return jsonify(response), 201

        #response = {'message': f'Block is full. Transactions will be not added to Block {index}. Mine first then add transactions to Block{index+1}'}
